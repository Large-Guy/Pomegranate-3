...
void Display::target(const std::weak_ptr<CommandQueue>& queue, const std::shared_ptr<Texture>& renderTexture) {
    ...
    if (this->texture == nullptr || renderTexture->width() != this->width || renderTexture->height() != this->height) {
        if (this->texture != nullptr) {
            SDL_DestroyTexture(this->texture);
        }
        this->width = renderTexture->width();
        this->height = renderTexture->height();
        this->texture = SDL_CreateTexture(this->sdlRenderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_STREAMING, width, height);
        int bytes = SDL_GetPixelFormatDetails(SDL_PIXELFORMAT_RGBA32)->bytes_per_pixel;
        this->transferBuffer = std::make_shared<TransferBuffer>(renderer, TransferBuffer::Type::Download, width * height * bytes);
    }
